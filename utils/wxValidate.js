...
class wxValidate {
	...
	__initMethods() {
      ...
      /**
			 ...
			 */
      phone(value) {
        return that.optional(value) || /\d{3}-\d{8}|\d{4}-\d{7}/.test(value)
      },
      /**
			 * 验证电话格式
			 */
      telephone(value) {
        return that.optional(value) || /((\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$)/.test(value)
      },
      ...
      imei(value, param){
        return that.optional(value) || /^[0-9]{15}$/.test(value)
      },
      bikeLicense(value, param) {
        return that.optional(value) || /[0-9A-z]$/.test(value)
      }
		}
	}
